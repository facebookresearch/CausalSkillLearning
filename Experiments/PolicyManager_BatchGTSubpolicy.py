...
#!/usr/bin/env python
from headers import *
from BatchPolicyNetworks import ContinuousPolicyNetwork, LatentPolicyNetwork, VariationalPolicyNetwork
...
class PolicyManager():
	...
	def __init__(self, number_policies=4, dataset=None, args=None):
		...
		# Set batch size. Basically just for speed. 
		...
		self.decay_counter = self.decay_epochs*(len(self.dataset)//self.batch_size)
	...
	def create_networks(self):
		...
		self.policy_network = ContinuousPolicyNetwork(self.input_size,self.hidden_size,self.output_size,self.number_policies, self.number_layers, batch_size=self.batch_size).cuda()

		# Create latent policy, whose action space = self.number_policies. 
		# This policy network automatically manages input size. 
		self.latent_policy = LatentPolicyNetwork(self.input_size, self.hidden_size, self.number_policies, self.number_layers, self.args.b_exploration_bias, batch_size=self.batch_size).cuda()

		# Create variational network. 
		self.variational_policy = VariationalPolicyNetwork(self.input_size, self.hidden_size, self.number_policies, number_layers=self.number_layers, z_exploration_bias=self.args.z_exploration_bias, b_exploration_bias=self.args.b_exploration_bias, batch_size=self.batch_size).cuda()
	...
	def update_plots(self, counter, subpolicy_loglikelihood, latent_loglikelihood, subpolicy_entropy, sample_traj, latent_z_logprobability, latent_b_logprobability):
		...
		self.writer.add_scalar('Total Likelihood', (subpolicy_loglikelihood+latent_loglikelihood).mean(), counter)
		self.writer.add_scalar('Epsilon', self.epsilon, counter)
		self.writer.add_scalar('Latent Z LogProbability', latent_z_logprobability.mean(), counter)
		self.writer.add_scalar('Latent B LogProbability', latent_b_logprobability.mean(), counter)
		...
		if counter%self.args.display_freq==0:
			self.writer.add_image("GT Trajectory",self.visualize_trajectory(sample_traj[0]), counter)
	...
	def assemble_inputs(self, input_trajectory, latent_z_indices, latent_b):
		assembled_inputs = torch.zeros((self.batch_size, input_trajectory.shape[1],self.input_size+self.number_policies+1)).cuda()
		assembled_inputs[:,:,:self.input_size] = torch.tensor(input_trajectory).view(self.batch_size, input_trajectory.shape[1] ,self.input_size).cuda().float()
		assembled_inputs[:,1:,self.input_size:-1] = torch.eye(self.number_policies)[latent_z_indices[:,:-1]]
		assembled_inputs[:,range(1,input_trajectory.shape[1]),-1] = latent_b[:,:-1].float()
	...
	def concat_state_action(self, sample_traj, sample_action_seq):
		...
		sample_action_seq = np.concatenate([np.zeros((self.batch_size,1,self.output_size)),sample_action_seq],axis=1)
	...
	def old_concat_state_action(self, sample_traj, sample_action_seq):
		sample_action_seq = np.concatenate([sample_action_seq, np.zeros((self.batch_size,1,self.output_size))],axis=1)
		return np.concatenate([sample_traj, sample_action_seq],axis=-1)
	...
	def evaluate_loglikelihoods(self, sample_traj, sample_action_seq, concatenated_traj, latent_z_indices, latent_b):
		...
		subpolicy_loglikelihoods = torch.zeros((sample_action_seq.shape[0],sample_action_seq.shape[1])).cuda()
		...
		for b in range(self.batch_size):
			for t in range(sample_action_seq.shape[1]):
				subpolicy_loglikelihoods[b,t] = self.dists[int(latent_z_indices[b,t])].log_prob(torch.tensor(sample_action_seq[b,t]).cuda().float())

		subpolicy_loglikelihood = subpolicy_loglikelihoods.sum(dim=1)*self.args.subpolicy_ratio
		subpolicy_entropy = torch.zeros(self.batch_size).cuda()
		...
		latent_b_logprobability = torch.zeros(self.batch_size).cuda()
		for t in range(self.batch_size):
			latent_b_logprobability[t] = latent_b_logprobabilities[t,range(sample_traj.shape[1]-1),latent_b[t,:-1].long()].sum()
		...
		diff_val = (1-(latent_z_indices==latent_z_indices.roll(1,1))[:,1:]).cuda().float()

		latent_z_logprobability = torch.zeros(self.batch_size).cuda()
		for b in range(self.batch_size):
			latent_z_logprobability[b] = torch.where(latent_b[b,:-1].byte(), latent_z_logprobabilities[b,range(sample_traj.shape[1]-1),latent_z_indices[b,:-1].long()], -self.lambda_likelihood_penalty*diff_val[b]).sum()
	...
	def update_policies(self, sample_action_seq, subpolicy_loglikelihoods, subpolicy_entropy, latent_b, latent_z_indices,\
		...
		self.latent_b_cross_entropy = self.negative_log_likelihood_loss_function(latent_b_logprobabilities.view(self.batch_size,2,-1), latent_b.long())
		self.latent_b_entropy = torch.bmm(latent_b_probabilities.view(self.batch_size,2,-1),latent_b_logprobabilities.view(self.batch_size,-1,2)).squeeze(1).squeeze(1).sum(dim=(1,2))

		self.latent_z_cross_entropy = self.negative_log_likelihood_loss_function(latent_z_logprobabilities.view(self.batch_size,self.number_policies,-1), latent_z_indices.long())
		self.latent_z_entropy = torch.bmm(latent_z_probabilities.view(self.batch_size,self.number_policies,-1),latent_z_logprobabilities.view(self.batch_size,-1,self.number_policies)).squeeze(1).squeeze(1).sum(dim=(1,2))
		...
		self.total_latent_loss = torch.where(latent_b.byte(),self.latent_b_loss+self.latent_z_loss,self.latent_b_loss).sum(dim=1)
		self.latent_policy_optimizer.zero_grad()
		self.total_latent_loss.mean().backward()
		...
		self.variational_b_cross_entropy = self.negative_log_likelihood_loss_function(variational_b_logprobabilities.view(self.batch_size,2,-1), latent_b.long())
		...
		self.variational_z_cross_entropy = self.negative_log_likelihood_loss_function(variational_z_logprobabilities.view(self.batch_size,self.number_policies,-1), latent_z_indices)
		...
		self.total_variational_loss = (self.variational_b_cross_entropy+self.variational_z_cross_entropy).sum(dim=1)
		# self.total_variational_loss = self.variational_z_cross_entropy
 
		if self.args.var_entropy:
			baseline_target = loglikelihood+self.total_variational_loss.mean()-1.
		else:
		...
		self.total_variational_loss.mean().backward(torch.ones_like(self.total_variational_loss).cuda()*(baseline_target-self.baseline))
	...
	def run_batch_iteration(self, counter, b):

		self.set_epoch(counter)

		############# (0) #############
		# Get batch_size number of samples to train on.
		sample_trajs, sample_action_seqs = self.dataset[b*self.batch_size:(b+1)*self.batch_size]

		start = 0

		if self.args.traj_length>0:
			sample_action_seqs = sample_action_seqs[:, start:self.args.traj_length-1]
		else:
			sample_action_seqs = sample_action_seqs[:, start:self.args.traj_length]
		...
		sample_trajs = sample_trajs[:, start:self.args.traj_length]

		# Now manage concatenated trajectory differently - {{s0,_},{s1,a0},{s2,a1},...,{sn,an-1}}.
		concatenated_trajs = self.concat_state_action(sample_trajs, sample_action_seqs)
		old_concatenated_trajs = self.old_concat_state_action(sample_trajs, sample_action_seqs)
		...
		latent_z_indices, latent_b, variational_b_logprobabilities, variational_z_logprobabilities,\
		variational_b_probabilities, variational_z_probabilities = self.variational_policy.forward(torch.tensor(old_concatenated_trajs).cuda().float(), self.epsilon)
		...
		latent_loglikelihood, latent_b_logprobabilities, latent_z_logprobabilities,\
		 latent_b_probabilities, latent_z_probabilities, latent_z_logprobability, latent_b_logprobability = self.evaluate_loglikelihoods(sample_trajs, sample_action_seqs, concatenated_trajs, latent_z_indices, latent_b)
		
		############# (3) #############
		# Update latent policy Pi_z with Reinforce like update using LL as return. 
		self.update_policies(sample_action_seqs, subpolicy_loglikelihoods, subpolicy_entropy, latent_b, latent_z_indices,\
			variational_z_logprobabilities, variational_b_logprobabilities, variational_z_probabilities, variational_b_probabilities,\
		...
		self.update_plots(counter, subpolicy_loglikelihood, latent_loglikelihood, subpolicy_entropy, sample_trajs, latent_z_logprobability, latent_b_logprobability)
	...
	def train(self, model=None):
		...
		# for e in range(1):
			...
			for b in range(self.num_batches):
				...
				self.run_batch_iteration(counter, b)
				counter = counter+1
