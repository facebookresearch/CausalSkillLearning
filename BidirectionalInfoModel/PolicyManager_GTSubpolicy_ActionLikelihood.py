...
class PolicyManager():
	...
	def create_networks(self):
		...
		# This policy network automatically manages input size. 
		self.latent_policy = LatentPolicyNetwork(self.input_size,self.hidden_size,self.number_policies, self.number_layers).cuda()

		# Create variational network. 
		self.variational_policy = VariationalPolicyNetwork(2*self.input_size, self.hidden_size, self.number_policies, number_layers=self.number_layers, exploration_bias=self.args.z_exploration_bias).cuda()
	...
	def update_plots(self, counter, subpolicy_loglikelihood, latent_loglikelihood, subpolicy_entropy, sample_traj):
	...
	def assemble_inputs(self, sample_traj, latent_z_indices, latent_b):
		...
		assembled_inputs = torch.zeros((len(sample_traj),self.input_size+self.number_policies+1)).cuda()
		assembled_inputs[range(len(sample_traj)),:self.input_size] = torch.tensor(sample_traj).view(len(sample_traj),self.input_size).cuda().float()
		assembled_inputs[range(len(sample_traj)),self.input_size+latent_z_indices] = 1.
	...
	def evaluate_loglikelihoods(self, sample_traj, sample_action_seq, latent_z_indices, latent_b):
		...
		assembled_inputs = self.assemble_inputs(sample_traj, latent_z_indices, latent_b)
		# concatenated_traj = self.concate_state_action(sample_traj, sample_action_seq)
		...
		latent_loglikelihood += latent_b_logprobabilities[range(len(sample_traj)-1),latent_b[:-1].long()].sum()
		...
		latent_loglikelihood += torch.where(latent_b[:-1].byte(), latent_z_logprobabilities[range(len(sample_traj)-1),latent_z_indices[:-1].long()], -self.lambda_likelihood_penalty*diff_val).sum()

		return subpolicy_loglikelihoods, subpolicy_loglikelihood, subpolicy_entropy, latent_loglikelihood, latent_b_logprobabilities, latent_z_logprobabilities, latent_b_probabilities, latent_z_probabilities
	...
	def update_policies(self, sample_action_seq, subpolicy_loglikelihoods, subpolicy_entropy, latent_b, latent_z_indices,\
		...
		if self.baseline is None:
			self.baseline = torch.zeros_like(loglikelihood).cuda().float()
		else:
		...
		# Gradient expression:		
		self.total_variational_loss.sum().backward(torch.ones_like(self.total_variational_loss).cuda()*(loglikelihood-self.baseline), retain_graph=True)		
		# self.total_variational_loss.backward(torch.ones_like(self.total_variational_loss).cuda()*(loglikelihood-self.baseline+self.total_variational_loss-1.), retain_graph=True)
	...
	def run_iteration(self, counter, i):
		...
		# The trajectory is going to be one step longer than the action sequence, because action sequences are constructed from state differences. 
		# Instead, truncate trajectory to length of action sequence. 
		sample_traj = sample_traj[start:len(sample_action_seq)]
		# sample_traj = sample_traj[start:max_length]		
		...
		latent_loglikelihood, latent_b_logprobabilities, latent_z_logprobabilities,\
		 latent_b_probabilities, latent_z_probabilities = self.evaluate_loglikelihoods(sample_traj, sample_action_seq, latent_z_indices , latent_b)
		...
		self.update_policies(sample_action_seq, subpolicy_loglikelihoods, subpolicy_entropy, latent_b, latent_z_indices,\
			variational_z_logprobabilities, variational_b_logprobabilities, variational_z_probabilities, variational_b_probabilities,\
			latent_z_logprobabilities, latent_b_logprobabilities, latent_z_probabilities, latent_b_probabilities, subpolicy_loglikelihood)

		# Update Plots. 
		# self.update_plots(counter, sample_map, loglikelihood)
		self.update_plots(counter, subpolicy_loglikelihood, latent_loglikelihood, subpolicy_entropy, sample_traj)
